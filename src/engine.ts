...
export class KittenTTSEngine {
  ...
  /** Buffers to destroy after inference completes (can't destroy while encoder is pending). */
   ...
   * Each dispatch gets its own compute pass (for memory barriers).
   * Dispatches accumulate until flushBatchEncoder() is called.
   */
  ...
  /** Defer buffer destruction until flushPendingDestroys() is called.
   *  Buffers may still be referenced by pending dispatches in the batch encoder. */
  private deferDestroy(buffer: GPUBuffer | undefined): void {
    if (buffer) this.pendingDestroys.push(buffer);
  }

  /** Destroy all deferred buffers. Call after inference completes. */
